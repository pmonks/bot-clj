...
(def git-url
  (str "https://github.com/pmonks/bot-clj/tree/" git-revision))
...
(defn reload!
  ...
  []
  (log/debug "Reloading clj-bot configuration...")
  (mnt/stop)
  (mnt/start)
  (log/debug "clj-bot configuration reloaded."))
...
(defn set-log-level!
  "Sets the log level (which can be a string or a keyword) of the bot, for the given package (a String, defaults to 'bot-clj', use 'ROOT' for the root logger)."
  ([level] (set-log-level! "bot-clj" level))
